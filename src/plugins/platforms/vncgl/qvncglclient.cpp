...
QVncGlClient::QVncGlClient(QTcpSocket *clientSocket, QVncGlServer *server)
    : QObject(server)
    , m_server(server)
...
QVncGlClient::~QVncGlClient()
{
    delete m_encoder;
}

QTcpSocket *QVncGlClient::clientSocket() const
{
    return m_clientSocket;
}

void QVncGlClient::setDirty(const QRegion &region)
{
...
void QVncGlClient::convertPixels(char *dst, const char *src, int count, int screendepth) const
{
    // cutoffs
...
void QVncGlClient::readClient()
{
    qCDebug(lcVncGl) << "readClient" << m_state;
    switch (m_state) {
        case Disconnected:
        ...
        case Protocol:
            if (m_clientSocket->bytesAvailable() >= 12) {
                ...
                proto[12] = '\0';
                qCDebug(lcVncGl, "Client protocol version %s", proto);
...
void QVncGlClient::discardClient()
{
    m_state = Disconnected;
    m_server->discardClient(this);
}

void QVncGlClient::checkUpdate()
{
    if (!m_wantUpdate)
        return;
#if QT_CONFIG(cursor)
...
void QVncGlClient::scheduleUpdate()
{
    if (!m_updatePending) {
...
bool QVncGlClient::event(QEvent *event)
{
...
void QVncGlClient::setEncodings()
{
    ...
    if (m_encodingsPending && (unsigned)m_clientSocket->bytesAvailable() >=
        ...
        for (int i = 0; i < m_encodingsPending; ++i) {
            ...
            enc = ntohl(enc);
            qCDebug(lcVncGl, "QVncGlServer::setEncodings: %d", enc);
            switch (enc) {
            case Raw:
                if (!m_encoder) {
                    m_encoder = new QRfbRawEncoder(this);
                    qCDebug(lcVncGl, "QVncGlServer::setEncodings: using raw");
                }
               break;
            case CopyRect:
    ...
    if (!m_encoder) {
        m_encoder = new QRfbRawEncoder(this);
        qCDebug(lcVncGl, "QVncGlServer::setEncodings: fallback using raw");
    }
}

void QVncGlClient::frameBufferUpdateRequest()
{
    qCDebug(lcVncGl) << "FramebufferUpdateRequest";
...
void QVncGlClient::pointerEvent()
{
    QRfbPointerEvent ev;
...
void QVncGlClient::clientCutText()
{
...
bool QVncGlClient::pixelConversionNeeded() const
{
    if (!m_sameEndian)
        return true;
